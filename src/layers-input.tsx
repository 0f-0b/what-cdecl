...
export function LayersInput({ name, root, layers, expected, onUpdate }: LayersInputProps): JSX.Element {
  return <>
    <div className={arrayEqual(layers.map(layer => layer.type), expected) ? "correct" : undefined}>
      instances of <code><span className="hl-variable">{name}</span></code> are <span>{joinArray(layers.map((layer, index): ReactNode => <button key={layer.id} className="layer removable" onClick={() => onUpdate([...layers.slice(0, index), ...layers.slice(index + 1)])}>{layerTypes[layer.type].description}</button>), " ")}</span> <code><span className="hl-type">{root}</span></code>s
    </div>
    <div>
